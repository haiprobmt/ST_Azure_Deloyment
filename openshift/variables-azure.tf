...
variable "azure_project" {
  type        = string
  description = "The project used for tagging."
  default     = "lta"
}
...
variable "azure_worker_availability_zones" {
  ...
  validation {
    condition     = length(var.azure_worker_availability_zones) == 1 || length(var.azure_worker_availability_zones) == 3
    error_message = "The azure_worker_availability_zones variable must be set to either [1] or [1, 2, 3] zones."
  }
}
